# Bugna Stratum Adapter

Modified from [@lukewarmIQ](https://github.com/shadowofnight98), this version replaces all references of Kaspa, KS_, kaspad; with: Bugna, bugna_ bugnad.
...
This is a lightweight daemon that allows mining to a local (or remote) bugna node using stratum-base miners.

This daemon is confirmed working with the miners below in both bugna-only and dual-mining modes (for those that support it) on Windows/MacOs/Linux/HiveOs.
...
- [@lukewarmIQ](https://github.com/shadowofnight98): `bugna:qraz34m4j5977nx6pnerf5uchq5cspafnng77eyyega8y0atgc4f2y46d8kuc`
- [@nullabit](https://github.com/nullabit): `bugna:qzzwu2lpqx7pepwvhssajq5ra32qylcxwcurqrgnjxrqqx6k6k6cvkffx3an2`

# Simpleguide:

-1. download sendrad.exe for the cli wallet. Run it with a bat file as `sendrad --utxoindex`. File found [here](https://github.com/bugnacoin/bugnad)
...
# HELP bugna_network_block_count Gauge representing the network block count
# TYPE bugna_network_block_count gauge
bugna_network_block_count 271966
# HELP bugna_network_difficulty_gauge Gauge representing the network difficulty
# TYPE bugna_network_difficulty_gauge gauge
bugna_network_difficulty_gauge 1.2526479386202519e+14
# HELP bugna_valid_share_counter Number of shares found by worker over time
# TYPE bugna_valid_share_counter counter
bugna_valid_share_counter{ip="192.168.0.17",miner="SRBMiner-MULTI/1.0.8",wallet="kaspa:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="002"} 276
bugna_valid_share_counter{ip="192.168.0.24",miner="BzMiner-v11.1.0",wallet="kaspa:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="003"} 43
bugna_valid_share_counter{ip="192.168.0.65",miner="BzMiner-v11.1.0",wallet="kaspa:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="001"} 307
# HELP bugna_worker_job_counter Number of jobs sent to the miner by worker over time
# TYPE bugna_worker_job_counter counter
bugna_worker_job_counter{ip="192.168.0.17",miner="SRBMiner-MULTI/1.0.8",wallet="kaspa:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="002"} 3471
bugna_worker_job_counter{ip="192.168.0.24",miner="BzMiner-v11.1.0",wallet="kaspa:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="003"} 3399
bugna_worker_job_counter{ip="192.168.0.65",miner="BzMiner-v11.1.0",wallet="kaspa:qzk3uh2twkhu0fmuq50mdy3r2yzuwqvstq745hxs7tet25hfd4egcafcdmpdl",worker="001"} 3425
...
* run `cd cmd/bugnabridge;go build .`

* Modify the config file in ./cmd/bridge/config.yaml with your setup, the file comments explain the various flags

* run `./bugnabridge` in the `cmd/bugnabridge` directory

All-in-one (build + run) `cd cmd/bugnabridge/;go build .;./bugnabridge`
...
* Enter the 'bugna-stratum-bridge' directory and type the command `docker compose -f docker-compose-bridge-src.yml up -d --build` [^2]. This will run the bridge assuming a local kaspad node with default port settings, and listen on port 5555 for incoming stratum connections.  These settings can be updated in the [config.yaml](cmd/bugnabridge/config.yaml) file, or overridden by modifying/adding/deleting the parameters in the 'command' section of the [docker-compose-bridge-src.yml](docker-compose-bridge-src.yml) file.  No further services will be enabled.

[^2]: This command builds the bridge component from source, rather than the previous behavior of pulling down a pre-built image.  You may still use the pre-built image by issuing the command `docker run -p 5555:5555 bugna_bridge:latest`, but it is not guaranteed to be up to date, so compiling from source is the better alternative.


# Configuration

Configuration for the bridge is done via the [config.yaml](cmd/bugnabridge/config.yaml) file in the same directory as the executable, or `./cmd/bugnabridge` from the project root if building from source / using docker.  Available parameters are as follows:
...
# bugnad_address: 46.17.104.200:16110
bugnad_address: localhost:16110
...
# block_wait_time: time to wait since last new block message from bugnad before
...
Config parameters can also be specificied by command line flags, which have slightly different names (these would be added in the 'command' subsection of the 'bugna_bridge' section of the appropriate 'docker-compose-*.yml' file for docker installations.)  This method has precedence over the config.yaml file:

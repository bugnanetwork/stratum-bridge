...
import (
	...
	"github.com/bugnanetwork/bugna-stratum-bridge/src/bugnastratum"
	"gopkg.in/yaml.v2"
)
...
func main() {
	...
	flag.StringVar(&cfg.RPCServer, "bugna", cfg.RPCServer, "address of the bugnad node, default `localhost:221100`")
	...
	log.Printf("\tBugna:          %s", cfg.RPCServer)
	...
	if err := bugnastratum.ListenAndServe(cfg); err != nil {
		log.Println(err)
	}
}
